...
> **개정(2026-07-01): HITL/게이트·챗 확인 제거, run 일괄 실행.** 초기 설계의 사람 확인
> 게이트(ask/pass)와 챗 세션 확인은 **제거**하고 나중으로 미룬다. 지금은 **입력 하나를 받아
> ReelProfile을 만들고 곧장 production까지 밀어붙이는 `run` 일괄 실행**이 기본이다. 규칙:
> (1) 입력이 이미 `ReelProfile` JSON이면 계획을 건너뛰고 **바로 production**을 실행한다.
> (2) 입력(텍스트/JSON, 내부에 로컬 경로·URL 포함 가능)에 **영상의 목적이 명확히 서술되지
> 않으면 실행을 거절**한다(exit≠0). (3) 목적이 있으면 그 목적만으로 캐릭터·제품·환경·음악
> 등 나머지를 추론해 채운다. 아래 본문의 "게이트/챗 확인" 서술은 이 개정으로 대체된다
> (챗 입력 자체는 추후 구현 가능하나 결국 run으로 수렴한다).
