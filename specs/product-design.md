...
> 나머지(캐릭터·환경·음악)는 추론해 채운다. 단, 제품은 임의 추정하지 않는다. 제품 URL/이미지
> 또는 구체적인 제품 설명이 없거나, 제품 URL을 읽지 못하면 `plan`/`run`은 실패하고 `chat`은 다시 묻는다.
>
...
- **`reel-gen plan <입력>`**: Planning 페이즈를 돈다. 입력(영상 목적 + 캐릭터/제품 + 선택
  ...
  `ReelProfile-{run_id}.json`을 산출한다. `run_id`는 입력 목적에서 뽑은 ASCII slug(최대 5단어)와
  생성 시각(`YYYYMMDD-HHMMSS`)으로 만든다. 한글 등 ASCII slug를 만들 수 없는 목적은
  `reel-YYYYMMDD-HHMMSS`가 될 수 있다. 챗 모드면 산출한 ReelProfile을 확인받고, 수정 요청이
  있으면 반영해 다시 만든다.
- **`reel-gen execute <ReelProfile.json>`**: Production 페이즈를 돈다. ReelProfile만 받아
  ...
  재생성을 건너뛴다(조립만). `storyboard.json`은 execute 입력이 아니라 verify의 교차 검증
  입력(`--storyboard`)이다.
...
+ 기대 효과 서술)가 돈다. 단독 CLI `verify`는 하드 pass/fail(fail이면 exit≠0)이다. 생성 그래프
안의 verify는 loudness fail만 최대 3회 assemble로 되돌려 재믹스하고, 통과하거나 교정이
불가능하거나 시도를 소진하면 다음 단계(describe/evaluate/report)로 진행한다. 이때 미해결 fail은
report에 남긴다. 즉 현재 production graph는 verify 미해결 fail만으로 즉시 종료하지 않는다.
`--no-vlm`이면 evaluate 노드는 실행되지만 rubric payload는 빈 dict로 남는다.

## 입력 형식

`run`과 `plan`의 위치 인자(`<입력>`)는 아래 형태를 받는다. 무엇이 들어왔는지는 시스템이 판별한다.
`chat`의 위치 인자는 같은 파서 계약이 아니라 선택적 시작 문장(`seed`)이다. `chat`은 그 seed를
대화 첫 사용자 발화로 넣고, 이후 목적·제품·캐릭터 등을 물어 채운다.

1. **`generation_input.json` 또는 느슨한 입력 파일 경로**: 제품, 스타일, 내러티브 등이 담긴
   상위 구조화 입력(`GenerationInput`, 스키마는 `src/reel_gen_agent/generate/schema.py`, 예:
   `demo/sample_input_1.json`)을 우선 지원한다. 단, 파일이 깨진 JSON 조각·메모·자연어일 수도
   있다고 보고, 유효한 `GenerationInput`이 아니면 텍스트 LLM이 영상 목적, 제품 정보, 제품 URL,
   캐릭터 유형, 스타일 기대 톤, 언어값을 한 번 정규화해 컨셉부터 전체 파이프라인을 태운다.
   유효한 `GenerationInput`의 `prompt`는 `run "자연어 브리프"` 입력과 같은 자유 요청으로 먼저
   보존하며, 그 안의 URL·레퍼런스·스타일 요구는 일반 intake 판별 규칙을 그대로 따른다.
   유효한 `GenerationInput`의 `product.path`는 JSON 파일 위치 기준의 로컬 제품 이미지 경로로
   해석하며, Product asset 생성의 참조 이미지로 반드시 주입한다. `model.path`도 JSON 파일 위치
   기준의 로컬 모델/캐릭터 이미지 경로로 해석해 Character asset 생성의 참조 이미지로 주입한다.
   `run`의 입력이 이미 `ReelProfile` JSON이면 계획을 건너뛰고 바로 Production으로 간다.
   `storyboard.json`이나 템플릿 JSON은 `execute` 입력이 아니다.
2. **따옴표로 감싼 텍스트 브리프**: 만들고 싶은 영상을 자연어로 적되, 그 안에 참고할 영상,
   제품, 캐릭터의 URL이나 로컬 경로를 섞어 넣는다. 컨셉 단계가 텍스트를 읽어 에셋을 뽑고
   각각을 레퍼런스 영상 / 제품 / 캐릭터 이미지로 분류해 Planning 상태를 만든다.

단일 에셋(이미지·영상·URL)만 넘기는 입력은 현재 `run`/`plan`에서 완성 입력이 아니다. 파일이나
URL의 역할은 판별할 수 있지만, 영상 목적 문장이 없으면 `validate_purpose`에서 거절된다. 에셋은
목적이 들어 있는 브리프 안에 함께 넣어야 한다.

URL과 로컬 경로는 어느 형태에서든 섞어 쓸 수 있다.

### 판별 규칙(구현 지침)

- `run`의 인자가 존재하는 `.json` 파일을 가리키고 `ReelProfile`로 검증되면 → 계획을 건너뛰고
  바로 Production을 실행한다. `GenerationInput`이면 자연어 브리프로 변환해 전체 파이프라인을
  탄다. 그 외 JSON/깨진 JSON/메모형 파일은 `execute` 입력으로 해석하지 않고, 텍스트 LLM으로
  목적·제품·캐릭터·스타일·언어를 정규화한 브리프를 만든 뒤 전체 파이프라인을 탄다.
- 인자가 존재하는 파일 경로이거나 단일 URL이고, 공백 없는 단일 토큰이면 → 에셋 역할은 추정할 수
  있지만, 목적 문장이 없으므로 `run`/`plan`은 거절한다. 영상은 레퍼런스, 비영상 URL은 제품 URL,
  로컬 이미지는 라벨에 따라 캐릭터/제품 참조로 분류된다.
...
- 에셋 역할 분류는 1차로 텍스트의 라벨("제품:", "레퍼런스 영상:", "캐릭터:" 등)을 따르고,
  ...
  YouTube/TikTok/Instagram/Vimeo URL은 직접 `.mp4` 확장자가 없어도 레퍼런스 영상으로 분류하고,
  `plan`/`run`이 `utils/add-reference.sh`를 통해 먼저 내려받은 로컬 mp4를 레퍼런스 시딩에 쓴다.
- 모호하거나 빠진 필드는 챗 모드에서 사용자에게 되묻는다. 런/플랜 모드는 영상 목적이 없으면
  거절하고, 제품을 확보하지 못하면 임의 추정하지 않고 실패한다. 목적과 제품이 확보되면 캐릭터,
  환경, 음악 등은 기본값과 모델 추론으로 채운다.
...
단일 에셋만 넘기면 목적 부재로 거절된다. 목적 문장 안에 레퍼런스를 함께 넣는다:

```bash
reel-gen run "이 세럼으로 15초 데일리 루틴 릴을 만들어줘. 레퍼런스 영상: ./reference_video/fast-cut.mp4"
```
...
- **생성 그래프의 "영상 결과물 확인" 단계**. 현재 자동 교정은 loudness fail만 대상으로 하고,
  교정 파라미터를 실어 assemble로 되돌려 재믹스한다(최대 3회). 통과하거나 교정 불가/소진이면
  describe로 진행하며 미해결 fail을 report에 남긴다. 생성 그래프 안에서는 미해결 conformance
  fail만으로 즉시 exit하지 않는다. visuals 등 다른 축 교정은 같은 틀에 향후 추가한다. 설계는
...
생성 그래프에서는 verify 단계 뒤에 실행된다. verify에 미해결 fail이 남아도 report 증거를 남기기
위해 evaluate까지 진행할 수 있다. `execute`/`run`에 `--no-vlm`을 주면 evaluate 노드는 지나가지만
rubric payload는 빈 dict다. 역할:
...
- 이미 생성된 에셋·클립이 있으면 재생성을 건너뛰고 조립만 한다. 필요한 재료가 없으면
  ProductionPlan에 따라 생성한다.
