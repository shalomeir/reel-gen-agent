...
- **storyboard**: 입력 + style_profile(컷 수/타이밍 시딩) + 내러티브 템플릿 + affordances
  + 환경 정의(EnvironmentSpec) + 플랫폼 제약을 합쳐 패널 목록과 콘티를 만든다. 각 패널은
  잠긴 환경을 참조하고(`environment_lock`), 플랫폼 세이프존/길이 상한/자막 가독성을
  프롬프트에 자동 주입한다. 비싼 영상 단계 전 게이트다.
- **gate: storyboard**: 패널 스틸과 타이밍을 image-to-video 비용 전에 승인한다.
