...
[trd.md](trd.md), 단계 내부의 세부 계약은 [../docs/pipeline-design.md](../docs/pipeline-design.md)에
있다. 이 문서가 그래프 구조의 정본이다. 스키마는 `src/reel_gen_agent/generate/schema.py`가
정본이고, 데이터 계약은 아래 "스키마" 절과 [information-schema.md](information-schema.md)에
정리한다. 그래프 설계가 초기안에서 어떻게 바뀌었는지 그 배경은 [ADR.md](ADR.md)와
[../retro.md](../retro.md)에 있다.
...
**hook ↔ storyboard 핑퐁.** storyboard 노드가 주어진 hook을 전체 스토리에
...
노드 순서는 위 mermaid와 `generate/plan_graph.py`가 정본이다. 컨셉 판단은 `reference_seed`
시딩과 각 노드의 LLM 결정이 나눠 맡고, 대사·자막·음악은 `storyboard`(자막), `narration`(대사),
`music`(마지막) 세 노드로 갈린다. style은 hook·story를 앞에서 이끌도록 초안 노드로 항상 채우고,
핑퐁이 끝난 뒤 확정된 서사에 맞춰 보정 노드를 한 번 더 둔다(레퍼런스 없을 때). 에셋 이미지는
`product`·`character` 노드가 각자 그 단계에서 만들고, `write_profile`이 그 산출과 환경 스펙을
`AssetBible`로 조립한다. 그래프 안 노드별 HITL 게이트는 아직 없어서, 누락 입력 확인은 그래프
밖 `chat`의 대화형 인테이크가 맡는다(아래 "게이트 동작" 참고).
...
- **environment (환경 정의)**: 목적·제품·캐릭터에 맞는 `EnvironmentSpec`(장소·조명·시간대·
  ...
  별도 환경 레퍼런스 이미지 생성은 향후로 미룬다([../retro.md](../retro.md)).
...
- **narration (대사 스크립트)**: storyboard 뒤에서 대사(나레이션)를 만든다. `delivery`는 기본
  ...
  `narrative_arc`는 패널 beat에서 만든다. 실제 TTS 생성은 execute의 `voice` 노드가 한다.
...
- **write_profile (에셋 조립 + 동결)**: `product`·`character` 노드가 만든
  `ProductProfile`·`CharacterProfile`과 environment 스펙을 `AssetBible`로 조립한다(에셋이 없으면
  이름만 든 프로필로 폴백). 이미지 경로는 plan/ 기준 상대명이라 ReelProfile이
...
`plan.bgm!=none`이면). **verify는 하드 게이트 + repair 유한 루프다**: conformance fail이 있으면
`plan_repair`가 `RepairAction`을 만들고 `verify->assemble` 백엣지로 되돌아가 재조립한다(주로
loudness 재믹싱, 최대 `MAX_REPAIR_ATTEMPTS=3`회, `repair.py`). 소진하거나 clean이면 describe로
진행한다.
...
flowchart TD
    ...
    bgm --> asm
    sfx --> asm
    asm --> verify[verify<br/>conformance 하드 게이트 + repair]
    verify -->|fail, 재시도 남음| asm
    verify -->|clean 또는 소진| describe[describe<br/>업로드용 제목·구조·멘트]
...
- **verify (conformance)**: 결과물이 의도한 템플릿대로 온전히 만들어졌고 노드 산출물이
  빠짐없이 머지됐는지 검증한다. 계약은 [conformance-gate.md](conformance-gate.md). **하드
  게이트 + repair 유한 루프로 동작한다.** conformance fail이 있으면 `plan_repair`가 어떤 결함을
  어떻게 고칠지 `RepairAction`으로 정하고, `verify->assemble` 백엣지로 되돌아가 재조립한다.
  현재 repair는 주로 loudness(음량) 재믹싱이며, 최대 `MAX_REPAIR_ATTEMPTS=3`회 시도한다
  (`repair.py`). 재시도를 소진하거나 clean이면 describe로 진행한다.
...
노드별 게이트다. verify는 이와 별개로 이미 하드 게이트 + repair 유한 루프로 동작한다(fail 시
`verify->assemble` 재조립, 최대 3회). 자세한 동작은 [product-design.md](product-design.md).
...
**향후 개선(이번 범위 밖, [../retro.md](../retro.md) 3절에 정리):**

1. 그래프 안 노드별 HITL 게이트 배선(단계별 확인/수정). 대화형 확인은 `chat`으로 이미 제공.
2. repair 확장: 현재 verify->assemble repair 루프는 loudness 재믹싱 위주다. 결함 종류별로
   해당 노드만 재생성하는 세분화된 `repair_router`로 넓히는 것은 향후 과제.
