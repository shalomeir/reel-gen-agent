...
> 아래 초기 설계에서 다음이 달라졌다.
> - **HITL/게이트 없음**: 확인 단계 없이 입력→ReelProfile→production 일괄(run).
...
> 아래 본문(머메이드·노드 서술·로드맵)은 이 개정에 맞춰 갱신됐다. 미룬 항목(HITL 게이트·
> chat·verify 하드+repair 루프 등)은 [../docs/Retrospective.md](../docs/Retrospective.md) 3절에 정리한다.
...
| `chat` (향후) | Planning + Production | 대화형 HITL 한 세션. 게이트와 함께 향후 개선으로 미룬다 |
...
flowchart TD
    ...
    narr --> write[write_profile<br/>에셋 조립 + ReelProfile 동결 plan/]
```
...
노드별 HITL 게이트(ask/pass/run)는 설계에만 있고 이번 범위에서는 미구현이다. 현재 plan
그래프는 게이트 없이 끝까지 돈다(run 모드와 동등). 게이트 프레임워크(`generate/gates.py`)는
있으나 그래프에 배선하지 않았다. 배선은 향후 개선으로 남긴다(아래 "게이트 동작" 참고).
...
`music`(hook 앞)·`storyboard`(자막)·`narration`(대사) 세 노드로 나뉜다. **에셋 이미지도
통합 `assets` 노드 없이 `product`·`character` 노드가 각자 그 단계에서 생성하고,
`write_profile`이 그 산출과 환경 스펙을 `AssetBible`로 조립한다.** 누락 입력 의도 확인과
노드별 HITL 게이트는 이번 범위에서 미구현이다(향후, 아래 "게이트 동작" 참고).
...
- **music (음악 정의)**: 장르·무드·다이내믹·prominence를 정한다(목적·제품·톤·페이싱·캐릭터
  + LLM). **hook보다 앞에서 돈다.** 실제 BGM 생성·믹스는 execute의 `bgm` 노드가 한다.
...
**상태: 설계만. 이번 범위 미구현(향후 개선).** 노드별 HITL 게이트(asset_bible, storyboard,
hook, scripting, confirm)와 대화형 `chat` 세션은 이번 프로젝트에서 구현하지 않는다. plan과
execute 그래프는 게이트 없이 끝까지 돈다(run 모드와 동등). 게이트 추상(`generate/gates.py`의
`GateConfig`/`resolve_gate`)은 준비돼 있으나 그래프에 배선하지 않았고, `run_planning`이 받는
`gate` 인자도 아직 쓰이지 않는다.

설계 의도(향후 배선 시): 모든 게이트는 같은 추상 위에서 동작한다. ask(챗 기본, 확인/수정),
pass(`--force-step-pass <step>`), 런 모드(전부 통과). verify는 게이트가 아니라 검증 단계다
(현재는 소프트 기록, 하드 pass/fail 루프는 향후). 자세한 동작은
...
- plan 페이즈 전체: intake -> reference_seed -> product(+제품 에셋) -> character(+캐릭터
  에셋) -> environment -> music -> hook <-> storyboard(핑퐁) -> narration -> write_profile.
  통합 assets 노드 없이 에셋을 노드 안에서 생성한다.
...
1. 노드별 HITL 게이트 배선과 대화형 `chat` 세션(게이트 추상 `gates.py`는 준비됨).
