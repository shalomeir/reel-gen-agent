...
| **fal-client** | 선택 이미지/영상 백엔드 | `FAL_KEY`가 있으면 FLUX.2 이미지 후보와 Kling O3 Standard/Seedance 2.0 Fast 영상 후보를 요청 시 provider로 쓴다 |
...
fal.ai(`FAL_KEY`)는 FLUX.2 이미지 생성과, 사용자가 공식 요청할 때의 Kling O3 Standard 또는
Seedance 2.0 Fast image-to-video에만 쓴다(Veo는 절대 fal로 호출하지 않는다). 영상 생성 모델 선택의
...
| 길이 | 18초(통상 15~22초) | 목적·후크·컨셉에 따라 조정 | 0초 초과 60초 이하. 60초 초과는 무조건 거부 |
| 프레임레이트 | 30fps | 백엔드 지원·후크 의도에 맞춰 24~60 표준값 | {24, 25, 30, 50, 60} 외 거부 |
| 컨테이너/코덱 | MP4 (H.264 영상 + AAC 오디오) | 없음 | mp4/H.264/AAC 고정 |

가드레일 규칙(밸리데이터가 강제):

- **길이**: `1 <= duration_sec <= 60`, 기본 18.0. 특별한 요구가 없으면 15초를 넘기고,
  15~22초가 가장 일반적인 대역이다. 60 초과는 거부한다("아주 길어도 60초를 넘지 않는다"가
  하드 상한). 후크·컨셉이 명시하면 그 대역 밖으로(더 짧게 또는 60초까지) 조정할 수 있다.
