...
| 영상 조립 | ffmpeg(시스템, subprocess) | 패널 클립 concat, 자막 오버레이, 음악 mux, 워터마크. moviepy 등 래퍼는 쓰지 않고 ffmpeg를 직접 호출한다 |
...
| 클라이언트 | 용도 | 비고 |
|---|---|---|
| **google-genai** | 비전 분석, 이미지 생성, Gemini API 호출, Vertex/Gemini Veo·Lyria·TTS 호출 | 백엔드는 `GENAI_BACKEND`로 고른다. 기본 `auto`는 Vertex 자격이 있으면 Vertex, 없으면 Gemini API key. 영상·음악·음성도 같은 규칙 |
| **google-cloud-storage** | Vertex Veo 출력 다운로드 | Vertex Veo 출력이 GCS로 떨어질 때 내려받기(Gemini lane은 File API라 불필요) |
| **anthropic** | 텍스트 생성(옵션 LLM) | 컨셉·훅·스토리보드·대사 텍스트를 Gemini 대신 Claude로 뽑는 대체 lane. `TEXT_MODEL_PRIORITY`가 고르고, `ANTHROPIC_API_KEY`가 있을 때만 쓴다 |
...
영상 백엔드 Veo 3.1은 **Vertex AI lane과 Gemini Developer API lane 둘 다** 쓴다. 어느 쪽을
쓸지는 다른 멀티모달 호출과 같은 `GENAI_BACKEND` 규칙으로 고른다(auto면 Vertex 우선, 자격이
없으면 Gemini). Vertex lane은 출력이 GCS로 떨어져 `GOOGLE_CLOUD_PROJECT`,
`GOOGLE_APPLICATION_CREDENTIALS`, `VEO_OUTPUT_GCS_URI`가 필요하고 GA 모델(`veo-3.1-*-001`)을
쓴다. Gemini API lane은 GCS 없이 File API로 바이트를 직접 받고 preview 모델(`GEMINI_VEO_MODEL`,
기본 `veo-3.1-fast-generate-preview`)을 쓴다. 그래서 `GEMINI_API_KEY` 하나만으로도 Veo 영상이
돈다(단일 키 약속이 영상까지 확장된다).
...
생성해 잇되 각 세그먼트는 해당 세그먼트 첫 패널 설정으로 만든 앵커 스틸을 start image로
쓴다. 이전 클립 마지막 프레임 재사용은 다음 세그먼트의 캐릭터·첫 컷 설정을 우회할 수 있어
기본 경로에서 쓰지 않는다. **BGM은 컷 주기에 bpm을 맞춰 생성**하고 컷-음악 동기를 별도 체크로
검증한다. 정본은 ai-model-records 4·5번.
...
- **사람 확인은 chat만**: 그래프 안 단계별 사람 확인 게이트는 두지 않는다. plan/run 경로는
  ...
  단계별 HITL 게이트 일반화(ask/pass/run)는 향후 과제다([../retro.md](../retro.md)).
...
google-genai, google-cloud-storage, anthropic, elevenlabs, fal-client, firecrawl-py,
langfuse, langgraph, pilmoji, yt-dlp, scenedetect, librosa, opencv-python, numpy,
pydantic, pillow, python-dotenv, typer, rich, prompt-toolkit, tenacity. 개발용으로
pytest, ruff, mypy. (영상 조립은 ffmpeg를 subprocess로 직접 호출하며 moviepy는 쓰지 않는다.)
...
3. 영상 단계를 `VideoBackend` 인터페이스 하나에 Veo 3.1 어댑터로 구현한다(Vertex lane과
   Gemini API lane 둘 다, `GENAI_BACKEND`로 선택). Kling O3 Standard/Seedance 2.0 Fast(fal
   lane)는 사용자 공식 요청 시 같은 인터페이스 뒤에 붙인다.
