...
- **기본 fal 경로는 Kling O3 Pro image-to-video로 고정한다.** 호출 모델 ID는
  `fal-ai/kling-video/o3/pro/image-to-video`이고, `VIDEO_MODEL_PRIORITY` 최상단에 둔다.
  `image_url`로 시작 이미지를 넣고 `end_image_url`로 끝 이미지를 넣을 수 있어 Veo 3.1
  image-to-video와 치환하기 쉽고, 명확한 A→B 액션 컷에서 특히 강하다. 우리 파이프라인은 컷마다
  Nano Banana로 start image를 만들어 주입하므로 이 경로와 잘 맞는다.
- **Kling O3 reference-to-video는 버린다(품질 미달).** 초기에는 여러 reference(캐릭터·제품·
  멀티샷 storyboard)를 한 번에 넣어 일관성을 잡는 최선 경로로 봤고, `elements`(frontal_image_url)
  기반 정체성 고정까지 붙여 봤다. 그러나 실제 생성 검증에서 **`fal-ai/kling-video/o3/pro/
  reference-to-video`의 영상 퀄리티가 기대에 크게 못 미쳤다**(2026-07-02 확인). 원하는 수준이
  안 나와 기본 경로에서 완전히 제외하고 `.env.example`의 우선순위·기본값에서도 뺐다. ref2v를
  다루는 코드는 남겨 두되(향후 모델 개선 시 재검토), 운영 경로로는 쓰지 않는다. 배경은
  [../retro.md](../retro.md) 참고.
...
- **10초 이상은 한 번에 어렵다**(모델별 상한). 더 길면 여러 번 생성해 이어 붙이되,
  각 세그먼트의 첫 패널 설정으로 만든 앵커 스틸을 그 세그먼트의 start image로 쓴다.
  이전 생성 영상의 마지막 프레임을 다음 start image로 재사용하지 않는다. 이전 프레임은
  구도 연속에는 유리하지만, 다음 세그먼트의 캐릭터·첫 컷 설정을 반영한 스틸을 우회해
  인물 드리프트를 키울 수 있다.
