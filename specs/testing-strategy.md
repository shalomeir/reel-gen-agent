...
conformance가 fail이면 rubric으로 가지 않고, 결함 카테고리로 해당 샷 재생성을 트리거한다.
두 게이트는 ask(챗 기본), pass(`--force-step-pass`), 런 모드(전부 통과)와 같은 그래프 위에서
동작한다. 흐름의 정본은 [conformance-gate.md](conformance-gate.md)와
[product-design.md](product-design.md)에 있다.
